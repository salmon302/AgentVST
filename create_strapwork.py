...
def generate_strapwork():
    base_dir = "examples/strapwork_counterpoint/StrapworkCounterpointWeaver"
    os.makedirs(os.path.join(base_dir, "src"), exist_ok=True)

    # 1. plugin.json
    plugin_json = {
      "plugin": {
        "name": "StrapworkCounterpointWeaver",
        "version": "1.0.0",
        "vendor": "AgentVST",
        "category": "Fx"
      },
      "parameters": [
        ...
        {
          "id": "over_under_ducking",
          "name": "Over/Under Ducking",
          "type": "float",
          "min": 0.0,
          "max": 100.0,
          "default": 75.0,
    ...
    PLUGIN_CODE "Strp"
    SOURCES
        src/StrapworkCounterpointWeaverDSP.cpp
)
target_include_directories(StrapworkCounterpointWeaver PRIVATE "${CMAKE_SOURCE_DIR}/framework/src")
"""
...
"""
    with open(os.path.join(base_dir, "src", "StrapworkCounterpointWeaverDSP.cpp"), "w") as f:
        f.write(dsp_content)

    print("Created StrapworkCounterpointWeaver files!")
