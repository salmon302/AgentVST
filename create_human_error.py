...
base_dir = "examples/human_error_ensemble/HumanErrorEnsemble"
os.makedirs(f"{base_dir}/src", exist_ok=True)

plugin_json = {
  "plugin": {
    "name": "HumanErrorEnsemble",
    "version": "1.0.0",
    "vendor": "AgentVST",
    "category": "Modulation"
  },
...
cmake = """agentvst_add_plugin(
    ...
    PLUGIN_CODE "HuEr"
    SOURCES
        src/HumanErrorEnsembleDSP.cpp
)
target_include_directories(HumanErrorEnsemble PRIVATE "${CMAKE_SOURCE_DIR}/framework/src")
"""
...
with open(f"{base_dir}/src/HumanErrorEnsembleDSP.cpp", "w") as f:
    f.write(dsp)

with open("examples/CMakeLists.txt", "a") as f:
